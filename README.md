...
**FlatGeobuf** is fully supported, both directions, built on a from-scratch
FlatBuffers reader and builder. All conversion routes through a shared feature
IR (`read(from) → FeatureCollection → write(to)`), so every format composes with
every other automatically: GeoJSON, GeoParquet, and FlatGeobuf convert to and
from each other in any combination, with no per-pair code. FGB is written
index-less (`index_node_size = 0`); GDAL/DuckDB read our output correctly in
both directions.
...
-   **More format spokes** — WKT/CSV are small, open, dependency-free next additions
    toward the any-to-any hub. GeoPackage (SQLite) is a larger one.
