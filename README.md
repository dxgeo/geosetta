...
**FlatGeobuf** can be read (`.fgb` → anything), built on a from-scratch
FlatBuffers reader; the write direction is not done yet. All conversion now
routes through a shared feature IR (`read(from) → FeatureCollection → write(to)`),
so any input format composes with any output format automatically — reading FGB
gives both FGB→GeoJSON and FGB→GeoParquet without a bespoke path.
...
-   **`flatbuffers.rs`:** a minimal, from-scratch FlatBuffers reader (vtables,
    offsets, vectors, strings, tables)
-   **`flatgeobuf/`:** FlatGeobuf reader — magic + header, spatial-index skip, and
    feature/geometry/property decode via `flatbuffers.rs`
-   **`geometry/`:** geometry model, bounding box, and WKB (Well-Known Binary)
    encoder/decoder — the shared geometry hub
...
-   **FlatGeobuf write direction** — the reader is done; the writer needs a
    from-scratch FlatBuffers **writer** (vtables/offsets/alignment) and, optionally,
    the packed Hilbert R-tree index. See [plans/flatgeobuf.org](plans/flatgeobuf.org).
-   **More format spokes** — WKT/CSV are small, open, dependency-free next additions
    toward the any-to-any hub.
