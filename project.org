...
- *Spatial indexing* — done across all three formats: the shared Hilbert
  ...
  should be the default is the one open question there.
...
*** External CRS resolution: =--print-crs-code= and =--crs= (implemented, 0.24.0)
...
- =geosetta <input> --print-crs-code= reports the source's identity as
  ...
  to report: no CRS at all, or a definition with no id in it. A GeoJSON or KML
  source is *not* that case — its spec fixes it at WGS 84, so it has a real
  identity that simply isn't written in the file, and it reports =OGC:CRS84=.
...
- *An id-less definition* (Esri-flavor =.prj= text, which carries no authority
  ...
  does exactly that (validating the name against the WKT's own ellipsoid), and
  its output is a definition, so it pipes straight in:
  ...
  adjudicate by eye. Geosetta needed no changes for any of this — =--crs=
  already takes arbitrary text; see
