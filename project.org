...
- *KML* and *KMZ* (read + write) on a from-scratch XML reader/writer
  ...
  always WGS 84 (KML has no other CRS concept). =.kmz= reading flattens every
  internal =*.kml= entry rather than trusting just the first, since a real
  multi-layer producer's root =doc.kml= can be nothing but a =<NetworkLink>=
  pointing at the actual data; writing packs the =.kml= bytes as one stored
  (uncompressed) zip entry — no DEFLATE encoder in the crate yet, so =.kmz=
  output is bigger than a compressed one but universally readable. Validated
  against real GDAL (=ogrinfo=, =ogr2ogr -f LIBKML=) and DuckDB-generated
  fixtures, plus a real Info-ZIP archive for the deflate read path.
...
([[https://github.com/dxgeo/geoscribe][=geoscribe=]]) so the
...
=tests/fixtures/gdal_native_geometry_{custom_name,3857}.parquet=). Nested/
repeated (list-valued) columns decode too, for the common case of a
single-level list of scalars (GDAL's =StringList=/DuckDB's
=LIST(VARCHAR)=) — the decoded array flows through the same JSON-text
fallback every format writer already uses for array-valued properties, with
*no* write-side code needed. Remaining gaps — a list of structs or a list
nested inside another list, 3D (Z/M) geometry, actually decoding (not just
detecting) multiple geometry columns, Brotli — are reported as clear errors
and tracked in
[[file:plans/arbitrary-geoparquet.org][plans/arbitrary-geoparquet.org]] (and,
for multiple geometry columns specifically,
...
- =crs.rs= / =crs/= :: the CRS intermediate representation and its resolution
  ...
  [[https://github.com/dxgeo/geoscribe][=geoscribe=]] crate). Wired in as an
  ...
  =crs/registry.rs= calls straight through to =geoscribe='s public API rather
  than keeping its own copy of the decoder: =geoscribe::resolve(authority,
  code)= for lookup by id (R1), =geoscribe::resolve_by_name(name)= for
  id-less-WKT name recovery, validated against ellipsoid params before being
  trusted (R2), and =geoscribe::CRS_COUNT= to sanity-check the embedded
  dataset size.
...
- *Resolve id-only CRS references via a runtime =geoscribe= fallback* — scoped
  in [[file:plans/crs-geoscribe-fallback.org][plans/crs-geoscribe-fallback.org]].
  Not a CRS-assignment flag (an earlier framing, corrected): a bare build
  (=--features crs-registry= off) that meets a source with an authority+code
  but no expanded definition warns instead of resolving it
  (=src/crs.rs:200=/=:232=); the fix is shelling out to =geoscribe='s CLI at
  runtime as a fallback, trading a compile-time dependency for an optional
  one. *Scoping only, not yet implemented.*
...
- *Deferred, lower-priority GeoParquet milestones* (parked in
  ...
  - 3D (Z/M) geometry — not started; a genuine IR-scope decision (does
    =Geometry=/=Position= grow a Z, touching every writer, or does the
    reader silently drop it), not just a decode-path addition. The plan
    file's milestone 6a.
  - Multiple geometry columns in one GeoParquet file — detected and
    rejected with a clear error (the plan file's milestone 6b); actually
    decoding and round-tripping one is scoped separately in
    [[file:plans/multi-geometry-columns.org][plans/multi-geometry-columns.org]],
    since it needs an IR change the reader alone can't make.
  - Nested/repeated (list-valued) columns — *done* for the common case (a
    single-level list of scalars, e.g. GDAL's =StringList=/DuckDB's
    =LIST(VARCHAR)=); a list of structs or a list nested inside another
    list still isn't decoded. The plan file's milestone 7.
...
The reverse direction — *PROJJSON→WKT*, for a GeoParquet source whose CRS
carries no authority code reaching FlatGeobuf, GeoPackage, or Shapefile's
=.prj= — is implemented the same way, structurally and without registry
involvement (the registry can't help here: an id-less PROJJSON has nothing to
key a lookup on). Geographic CRSes translate directly; projected CRSes reuse
=wkt1_tables.rs='s crosswalk in reverse (an EPSG method/parameter code back to
its GDAL WKT1 name), so a custom/local datum combined with a standard,
catalogued projection method — confirmed a realistic case against real
=ogr2ogr= output, not just hypothetical — still reaches a WKT-dialect target
instead of dropping the CRS. Verified against real =projinfo --identify= (both
an all-metre and a US-survey-foot projected case, each at 100%) and against
real =ogrinfo= reading a Geosetta-written =.prj= end to end
...
[[https://github.com/dxgeo/geoscribe][=geoscribe=]] crate so the
...
*Reprojection composability (implemented).* Because the CRS is a public value
on the IR and geometry is stored apart from it, a reprojection tool slots in
as a stage between read and write — transforming coordinates and updating
=Crs= — without Geosetta itself ever learning projection math (scoped in
...
- *A linked-in library* uses
  ...
  (=src/feature.rs=, =src/geometry/mod.rs=) to rewrite every coordinate in
  place — the =_run_= variant yields whole contiguous coordinate runs (a ring,
  a =LineString=, ...) rather than one point at a time, so a backend that
  batches (PROJ's =proj_trans_array=, a SIMD kernel) gets a real batch instead
  of a per-point callback.
- *An external process* — including one with no Rust API to link against at
  all — composes through =-= as =<input>=/=<output>=, the standard stdin/stdout
  placeholder (needs =--from=/=--to= explicitly, since there's no path to infer
  a format from): =reproject-tool --to EPSG:3857 < in.geojson | geosetta
  --from geojson --to fgb - out.fgb=. Not accepted for Shapefile — sibling
  =.shp=/=.shx=/=.dbf=/=.prj= files, not a single byte stream — which routes
  through a single-buffer format instead (FlatGeobuf and GeoParquet both
  carry the CRS faithfully) around the external step.

Verified against three independent, real tools in =tests/reproject_pipe.rs=:
GDAL (=ogr2ogr=, piped, container- and CRS-aware), PROJ (=cs2cs=, piped, a bare
...
*library* with no CLI binary, linked directly against =for_each_position_mut=
as a dev-dependency). All three transform the same fixture to EPSG:3857 and
