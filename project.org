...
- =crs.rs= :: the format-neutral coordinate-reference-system identity carried on
  ...
  to writer — Geosetta labels, but never reprojects. Includes lexical id lifting
  from a WKT/PROJJSON definition and the =Crs::downgrade_warning= CRS-loss
  warnings. =crs/wkt_projjson.rs= structurally translates a WKT CRS definition
  into resolvable GeoParquet PROJJSON (geographic, WKT2 projected, and the common
  WKT1 projected methods via the crosswalk tables in =crs/wkt1_tables.rs=)
