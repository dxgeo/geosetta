...
- =crs.rs= :: the format-neutral coordinate-reference-system identity carried on
  ...
  into resolvable GeoParquet PROJJSON (geographic CRSes)
...
into a complete PROJJSON object* (=src/crs/wkt_projjson.rs=, geographic CRSes) so
the GeoParquet output resolves correctly — a GeoPackage's =EPSG:7844= (GDA2020)
reaches GeoParquet as a full =GeographicCRS= PROJJSON, emitted in the
authoritative latitude,longitude axis order so GDAL/QGIS *identify it as
EPSG:7844* at 100% confidence (a longitude-first order would identify at only
~25% and show a bare name). GeoParquet's fixed x,y coordinate storage is honored
by readers independently of the CRS axis order, so geometry is not flipped. When there is *no* WKT to translate (only a bare code) or the WKT is a
kind not yet translated (projected CRSes), Geosetta writes the id-only reference
as a last resort — it preserves the identity for Geosetta's own round trip but
is unresolvable downstream, so the CLI *warns*. Geosetta still never parses these
